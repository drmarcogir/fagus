...
fitsem_wrapper1<-function(.){
  ...
  sr1pool=list(
    ...
    m2<-glm(SR1~LGMS+DIST+TOPO+ATEMP+APREC+SPREC+PLOT+PH+L+T+M+N+R+Rpool1,data=dat.rpool1,family=poisson),
  ...
  sr2pool=list(
    ...
    m2<-glm(SR2~LGMS+DIST+TOPO+ATEMP+APREC+SPREC+PLOT+PH+L+T+M+N+R+Rpool2,data=dat.rpool2,family=poisson),
